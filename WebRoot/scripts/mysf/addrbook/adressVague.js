        ...
        $("#user_address").onkeyup = keydown;
		//输入提示
		function autoSearch() {
			var city=$("#user_show_all").text();
			var str ="";
			str=city.substring(city.indexOf(" "), city.lastIndexOf(" "));
			str=$.trim(str);
		    var user_addresss = $("#user_address").text();
		    var auto;
		    //加载输入提示插件
		...
		function selectResult(index) {
		    ...
		    if (navigator.userAgent.indexOf("MSIE") > 0) {
		        document.getElementById("user_address").onpropertychange = null;
		        document.getElementById("user_address").onfocus = focus_callback;
		    }
		    //截取输入提示的关键字部分
		    var text = document.getElementById("divid" + (index + 1)).innerHTML.replace(/<[^>].*?>.*<\/[^>].*?>/g,"");
			var cityCode = document.getElementById("divid" + (index + 1)).getAttribute('data');
			$("#user_address").text(text);
		    document.getElementById("result1").style.display = "none";
		    $("#cleanNum").show();
		    //根据选择的输入提示关键字查询
		...
		function focus_callback() {
		    if (navigator.userAgent.indexOf("MSIE") > 0) {
		        document.getElementById("user_address").onpropertychange = autoSearch;
		   }
		}
		...
		function keydown(event){
		    ...
		    if(key===40){//down
		        if(cur + 1 < result.childNodes.length){
		            ...
		            $("#user_address").text(result.tipArr[cur+1].name);
		        }
		    }else if(key===38){//up
		        if(cur-1>=0){
		            if(result.childNodes[cur]){
